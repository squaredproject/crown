...
class Fireflies extends TSTriggerablePattern {
  ...
  public void run(double deltaMs) {
    ...
    for (Firefly fly:fireflies) {
      for (Bulb bulb: model.bulbs) {
        if (Utils.abs(fly.yPos - bulb.transformedY) <= radius && Utils.abs(fly.theta - bulb.transformedTheta) <= radius) {
          float distSq = Utils.pow((LXUtils.wrapdistf(fly.theta, bulb.transformedTheta, 360)), 2) + Utils.pow(fly.yPos - bulb.transformedY, 2);
          float brt = Utils.max(0, 100 - Utils.sqrt(distSq * 4) - blinkers[fly.blinkIndex].getValuef());
          if (brt > LXColor.b(colors[bulb.index])) {
            colors[bulb.index] = lx.hsb(
...
class Lattice extends TSPattern {
  ...
  public void run(double deltaMs) {
    ...
    for (Bulb bulb : model.bulbs) {
      float wrapdistleft = LXUtils.wrapdistf(bulb.transformedTheta, spinf + (model.yMax - bulb.transformedY) * coilf, 180);
      float wrapdistright = LXUtils.wrapdistf(bulb.transformedTheta, -spinf - (model.yMax - bulb.transformedY) * coilf, 180);
      float width = yClimb.getValuef() + ((bulb.transformedY - yHeight.getValuef())/model.yMax) * 50;
      float df = Utils.min(100, 3 * Utils.max(0, wrapdistleft - width) + 3 * Utils.max(0, wrapdistright - width));

      colors[bulb.index] = lx.hsb(
        (hue.getValuef() + lx.getBaseHuef() + .2f*bulb.transformedY - 360) % 360, 
        100, 
        df
      );
    }
...
class Fire extends TSTriggerablePattern {
  ...
  public void run(double deltaMs) {
    ...
    for (Bulb bulb: model.bulbs) {
      float yn = (bulb.transformedY - model.yMin) / model.yMax;
      float cBrt = 0;
      float cHue = 0;
      float flameWidth = flameSize.getValuef() / 2;
      for (int i = 0; i < flames.size(); ++i) {
        if (Utils.abs(flames.get(i).theta - bulb.transformedTheta) < (flameWidth * (1- yn))) {
          cBrt = Utils.min(100, Utils.max(0, Utils.max(cBrt, (100 - 2 * Utils.abs(bulb.transformedY - flames.get(i).decay.getValuef()) - flames.get(i).decay.getBasisf() * 25) * Utils.min(1, 2 * (1 - flames.get(i).decay.getBasisf())) )));
...
class Bubbles extends TSTriggerablePattern {
  ...
  public void run(double deltaMs) {
    ...
    for (Bubble bubble: bubbles) {
      for (Bulb bulb : model.bulbs) {
        if (Utils.abs(bubble.theta - bulb.transformedTheta) < bubble.radius && Utils.abs(bubble.yPos - (bulb.transformedY - model.yMin)) < bubble.radius) {
          float distTheta = LXUtils.wrapdistf(bubble.theta, bulb.transformedTheta, 360) * 0.8f;
          float distY = bubble.yPos - (bulb.transformedY - model.yMin);
          ...
          if (distSq < bubble.radius * bubble.radius) {
            ...
            colors[bulb.index] = lx.hsb(
              ...
              Utils.constrain(bulb.transformedY/model.yMax * 125 - 50 * (dist/bubble.radius), 0, 100)
            );
          }
        }
      }
...
class Voronoi extends TSPattern {
  ...
  public void run(double deltaMs) {
    ...
    for (Bulb bulb: model.bulbs) {
      ...
      for (int i = 0; i < sites.length; ++i) {
        if (Utils.abs(sites[i].yPos - bulb.transformedY) < 150) { //restraint on calculation
          float distSq = Utils.pow((LXUtils.wrapdistf(sites[i].theta, bulb.transformedTheta, 360)), 2) + Utils.pow(sites[i].yPos - bulb.transformedY, 2);
...
class Pulley extends TSTriggerablePattern { //ported from SugarBulbs
  ...
  public void run(double deltaMS) {
    ...
    if(triggered) {
      ...
      for (Bulb bulb : model.bulbs) {
        int gi = (int) Utils.constrain((bulb.x - model.xMin) * NUM_DIVISIONS / (model.xMax - model.xMin), 0, NUM_DIVISIONS-1);
        float yn =  bulb.transformedY/model.yMax;
        colors[bulb.index] = lx.hsb(
          (lx.getBaseHuef() + Utils.abs(bulb.x - model.cx)*.8f + bulb.transformedY*.4f) % 360, 
          Utils.constrain(100 *(0.8f -  yn * yn), 0, 100), 
          Utils.max(0, 100 - Utils.abs(bulb.transformedY/2 - 50 - gravity[gi].getValuef())*falloff)
        );
      }
    }
  }
...
class Springs extends TSPattern {
  ...
  public void run(double deltaMS) {
    ...
    for (Bulb bulb : model.bulbs) {
      float yn =  bulb.transformedY/model.yMax;
      float width = (1-yn) * 25;
      float wrapdist = LXUtils.wrapdistf(bulb.transformedTheta, spinf + (bulb.transformedY) * 1/(gravity.getValuef() + 0.2f), 360);
...
class Pulleys extends TSTriggerablePattern { //ported from SugarBulbs
  private BasicParameter sz = new BasicParameter("SIZE", 0.5);
  private BasicParameter beatAmount = new BasicParameter("BEAT", 0);
  private BooleanParameter automated = new BooleanParameter("AUTO", true);
  private BasicParameter speed = new BasicParameter("SPEED", 1, -3, 3);
  ...
  public void run(double deltaMS) {
    ...
    for (Bulb bulb : model.bulbs) {
      ...
      float cHue = 0;
      for (int j = 0; j < pulleys.size(); ++j) {
        cHue = (lx.getBaseHuef() + Utils.abs(bulb.x - model.cx)*.8f + bulb.transformedY*.4f + pulleys.get(j).baseHue) % 360;
        cBrt += Utils.max(0, pulleys.get(j).maxBrt.getValuef() * (100 - Utils.abs(bulb.transformedY/2 - 50 - pulleys.get(j).gravity.getValuef())*falloff));
      }
      float yn =  bulb.transformedY/model.yMax;
      colors[bulb.index] = lx.hsb(
        cHue, 
...
//    for (Bulb bulb: model.bulbs) {
//      bulb.transformedY = bulb.transformedY * ( 1 - ripple.getValuef() * amplitude.getValuef() * Utils.sin((bulb.transformedTheta + rotate) / 30 * Utils.PI ));
//    }
//  }
//}
...
//     for (Bulb bulb : model.bulbs) {
//       float distVal = Utils.sqrt(Utils.pow((LXUtils.wrapdistf(thetaCenter, bulb.transformedTheta, 360)) * 0.8f, 2) + Utils.pow(yCenter - bulb.transformedY, 2));
//       float heightHueVariance = 0.1f * bulb.transformedY;
//       if (distVal < radius){
...
//       }
//       colors[bulb.index] = lx.hsb(hueVal,  100, brightVal);
//     }
//   }
// }
