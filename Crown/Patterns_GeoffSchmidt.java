...
class Pixels extends TSPattern {
  ...
  Pixels(LX lx) {
    ...
    int numBulbs = model.bulbs.size();
    pixelStates = new PixelState[numBulbs];
    for (int n = 0; n < numBulbs; n++)
...
class Wedges extends TSPattern {
  ...
  public void run(double deltaMs) {
    ...
    for (Bulb bulb : model.bulbs) {
      colors[bulb.index] = LXColor.hsb(
        Math.floor((rotation - bulb.transformedTheta) / quant) * quant + vHue * 360.0f,
        (1 - vSat) * 100,
        100);
    }     
  } 
  ...
  public void run(double deltaMs) {
    ...
    for (Bulb bulb : model.bulbs) {
      ...
      for (ColorBar colorBar : colorBars) {
        if (colorBar.intersects(bouncedNow, bulb.transformedY)) {
          colors[bulb.index] = colorBar.getColor(pHue.getValuef() * 360);
          break;
        }
      }
    }     
