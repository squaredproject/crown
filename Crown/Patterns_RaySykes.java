...
class SparkleHelix extends TSPattern {
  ...
  SparkleHelix(LX lx) {
    ...
    sparkleTimeOuts = new int[model.bulbs.size()];
  }
  ...
  public void run(double deltaMs) {
    ...
    for (Bulb bulb : model.bulbs) {
      ...
      float spiralVal = Utils.max(0, 100 - (100*Utils.TWO_PI / (compensatedWidth))*LXUtils.wrapdistf((Utils.TWO_PI / 360) * bulb.transformedTheta, 8*Utils.TWO_PI + spin.getValuef() + coil.getValuef()*(bulb.transformedY-model.cy), Utils.TWO_PI));
      float counterSpiralVal = counterSpiralStrength.getValuef() * Utils.max(0, 100 - (100*Utils.TWO_PI / (compensatedWidth))*LXUtils.wrapdistf((Utils.TWO_PI / 360) * bulb.transformedTheta, 8*Utils.TWO_PI - spin.getValuef() - coil.getValuef()*(bulb.transformedY-model.cy), Utils.TWO_PI));
      float hueVal = (lx.getBaseHuef() + .1f*bulb.transformedY) % 360;
      if (sparkleTimeOuts[bulb.index] > Utils.millis()){        
        colors[bulb.index] = lx.hsb(hueVal, sparkleSaturation.getValuef(), 100);
      }
      else{
        colors[bulb.index] = lx.hsb(hueVal, 100, Utils.max(spiralVal, counterSpiralVal));        
        if (Utils.random(Utils.max(spiralVal, counterSpiralVal)) > sparkle.getValuef()){
          sparkleTimeOuts[bulb.index] = Utils.millis() + 100;
        }
      }
    }
  }
...
class MultiSine extends TSPattern {
  ...
  public void run(double deltaMs) {
    ...
    for (Bulb bulb : model.bulbs) {
      ...
      for (int i = 0; i < numLayers; i++){
        combinedDistanceSines[0] += Utils.sin(Utils.TWO_PI * frequencies[i].getValuef() + bulb.transformedY / distLayerDivisors[0][i]) / numLayers;
        combinedDistanceSines[1] += Utils.sin(Utils.TWO_PI * frequencies[i].getValuef() + Utils.TWO_PI*(bulb.transformedTheta / distLayerDivisors[1][i])) / numLayers;
      }
      ...
      colors[bulb.index] = lx.hsb(hueVal,  satVal, brightVal);
    }
  }
}
...
class Stripes extends TSPattern {
  ...
  public void run(double deltaMs) {
    ...
    for (Bulb bulb : model.bulbs) {  
      float hueVal = (lx.getBaseHuef() + .1f*bulb.transformedY) % 360;
      float brightVal = 50 + 50 * Utils.sin(spacing.getValuef() * (Utils.sin((Utils.TWO_PI / 360) * 4 * bulb.transformedTheta) + slopeFactor.getValuef() * bulb.transformedY)); 
  ...
  public void run(double deltaMs) {
    ...
    if (coverage.getValuef() < 5){
      if (!resetDone){
        ...
        for (int i = 0; i < model.bulbs.size(); i++){
          sparkleTimeOuts[i] = 0;
        }        
        resetDone = true;
      }
    ...
    for (Bulb bulb : model.bulbs) {  
      float newHueVal = (lx.getBaseHuef() + complimentaryToggle * hueSeparation + hueVariation.getValuef() * bulb.transformedY) % 360;
      float oldHueVal = (lx.getBaseHuef() + lastComplimentaryToggle * hueSeparation + hueVariation.getValuef() * bulb.transformedY) % 360;
      if (sparkleTimeOuts[bulb.index] > Utils.millis()){        
        colors[bulb.index] = lx.hsb(newHueVal,  (30 + coverage.getValuef()) / 1.3f, newBrightVal);
      }
      else {
        colors[bulb.index] = lx.hsb(oldHueVal,  (140 - coverage.getValuef()) / 1.4f, oldBrightVal);
        float chance = Utils.random(Utils.abs(Utils.sin((Utils.TWO_PI / 360) * bulb.transformedTheta * 4) * 50) + Utils.abs(Utils.sin(Utils.TWO_PI * (bulb.transformedY / 9000))) * 50);
        if (chance > (100 - 100*(Utils.pow(coverage.getValuef()/100, 2)))){
          sparkleTimeOuts[bulb.index] = Utils.millis() + 50000;
        }
...
class Lightning extends TSTriggerablePattern {
  private int nBolts = 5; // larger than towers
  private LightningLine[] bolts = new LightningLine[nBolts];
    ...
    for (Tower tower : model.towers){
      if (triggered) {
        if (bolts[towerIndex].isDead()) {
          if (firesOnBeat.isOn()) {
            if (lx.tempo.beat()) {
              randomCheckTimeOuts[towerIndex] = Utils.millis() + 100;
              bolts[towerIndex] = makeBolt();
            }
          } else {
            if (randomCheckTimeOuts[towerIndex] < Utils.millis()){
              randomCheckTimeOuts[towerIndex] = Utils.millis() + 100;
      ...
      for (Bulb bulb : tower.bulbs) {
        float hueVal = 300;
        float lightningFactor = bolts[towerIndex].getLightningFactor(bulb.transformedY, bulb.transformedTheta);
...
class IceCrystals extends TSPattern {
  ...
  public void run(double deltaMs) {
    ...
    crystal.doUpdate();
    for (Bulb bulb : model.bulbs) {
      float lineFactor = crystal.getLineFactor(bulb.transformedY, bulb.transformedTheta);
      if (lineFactor > 110) {
