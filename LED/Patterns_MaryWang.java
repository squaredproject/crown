...
class Twinkle extends TSPattern {
  ...
  Twinkle(LX lx) {
    ...
    bulbToModulatorMapping = new int[model.bulbs.size()];

    for (int i = 0; i < bulbToModulatorMapping.length; i++ ) {
      bulbToModulatorMapping[i] = (int)Utils.random(numBrights);
    }    
    ...
    for (Bulb bulb : model.bulbs) {
      if (sparkleTimeOuts[bulb.index] < Utils.millis()) {
        ...
        if (Utils.random(10) <= 3) {
          bulbToModulatorMapping[bulb.index] = (int)Utils.random(numBrights);
        }
        sparkleTimeOuts[bulb.index] = Utils.millis() + (int)Utils.random(11100, 23300);
      }
      colors[bulb.index] = lx.hsb(
      0, 
      0, 
      bright[bulbToModulatorMapping[bulb.index]].getValuef() * brightnessParam.getValuef()
        );
    }
  }
}
...
class VerticalSweep extends TSPattern {
  ...
  public void run(double deltaMs) {
    ...
    for (Bulb bulb : model.bulbs) {
      float progress = ((bulb.transformedTheta / 360.0f) + range.getValuef()) % 1; // value is 0-1
      ...
      colors[bulb.index] = LXColor.lerp(color1, color2, amt);
    }
  }
}
