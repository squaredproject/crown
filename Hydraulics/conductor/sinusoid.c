...
static int32_t theta[NUMBER_FINGERS][KNUCKLES_PER_FINGER] = {
    {0}}; // scaled such that theta = 1024 = 2 * PI rad

static int32_t pacer_period = 0; // samples
static uint8_t frequency[NUMBER_FINGERS][KNUCKLES_PER_FINGER] = {
    {0}}; // expressed relative to pacer in 1/16ths: f_knuckle = frequency / (16
          // * pacer_period)
static int32_t phase[NUMBER_FINGERS][KNUCKLES_PER_FINGER] = {
    {0}}; // for max = 512?

static int32_t position[NUMBER_FINGERS][KNUCKLES_PER_FINGER] = {
    {0}}; // for max = +/- 255
static int32_t amplitude[NUMBER_FINGERS][KNUCKLES_PER_FINGER] = {
    {0}}; // for max = +/- 255
static int32_t amplitude_des[NUMBER_FINGERS][KNUCKLES_PER_FINGER] = {
    {0}}; // for max = +/- 255
...
void apply_ui_to_sinusoids(int32_t knob[8], uint8_t mode) {
  ...
  apply_ui_modes(mode, pacer_theta, knob, amplitude_des, phase, frequency,
...
void pace_sinusoids(void) {
  ...
  for (finger = 0; finger < NUMBER_FINGERS; finger++) {
    for (knuckle = 0; knuckle < KNUCKLES_PER_FINGER; knuckle++) {
      int32_t ideal_increment =
          (this_pacer_increment * frequency[finger][knuckle]) >> 4;
      if (ideal_increment < 1)
        ideal_increment = 1;

      int32_t theta_des = ((frequency[finger][knuckle] * pacer_theta) >> 4) +
                          phase[finger][knuckle];

      theta_des = mod_table(theta_des, 16);

      int32_t theta_error =
          mod_table((theta_des - theta[finger][knuckle] + 8 * TABLE_LENGTH),
                    16) -
          8 * TABLE_LENGTH;
      ...
      int32_t increment_floor =
          ideal_increment >> 1; // worst case = 0, acceptable
...
void generate_positions(void) {
  ...
  for (finger = 0; finger < NUMBER_FINGERS; finger++) {
    for (knuckle = 0; knuckle < KNUCKLES_PER_FINGER; knuckle++) {
      int32_t amplitude_error =
          amplitude_des[finger][knuckle] - amplitude[finger][knuckle];
      ...
      if (direct_position_mode == 1) {
        position[finger][knuckle] =
            amplitude[finger][knuckle] + CENTER_POSITION;
      } else {
        position[finger][knuckle] =
            sine_table(theta[finger][knuckle], amplitude[finger][knuckle]) +
            CENTER_POSITION;
      }
      // FIELD HACK - reduce the amplitude by half
      // CSW - removing FIELD hack for a while...
      //			position[finger][knuckle] =
      //position[finger][knuckle] / 2;
...
void stream_positions(void) {
  ...
  for (finger = 0; finger < NUMBER_FINGERS; finger++) {
    for (knuckle = 0; knuckle < KNUCKLES_PER_FINGER; knuckle++) {
      fprintf(NETWORKFILE, "<%u%ut%04d>", finger + 1, knuckle + 1,
              (int)position[finger][knuckle]);
    }
  }
  fprintf(NETWORKFILE, "\r\n");
}
    ...
    for (finger = 0; finger < NUMBER_FINGERS; finger++) {
      for (knuckle = 0; knuckle < KNUCKLES_PER_FINGER; knuckle++) {
        ...
        case SINE_DEBUG_POSITIONS:
          printf("<%u%ut%04d>", finger + 1, knuckle + 1,
                 (int)position[finger][knuckle]);
          break;

        case SINE_DEBUG_THETA:
          printf("<%u%ut%04d>", finger + 1, knuckle + 1,
                 (int)theta[finger][knuckle]);
          break;
        }
      }
    }
