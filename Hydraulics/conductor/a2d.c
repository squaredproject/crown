...
#define LOW_PASS_BITS                                                          \
  6 // exponential moving average over 2^LOW_PASS_BITS samples
...
void A2D_poll_adc(void) {
  ...
  if (bit_is_clear(ADCSRA, ADSC)) { // if clear, conversion is done.
    ...
    ADC_low_pass[chanNum] = ADC_low_pass[chanNum] -
                            (ADC_low_pass[chanNum] >> LOW_PASS_BITS) +
                            ADValue[chanNum];
