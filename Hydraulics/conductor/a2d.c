...
static uint32_t ADC_low_pass[8];
...
#define LOW_PASS_BITS 6		// exponential moving average over 2^LOW_PASS_BITS samples
...
void A2D_Init(void)
{
  ...
  for (n=0;n<8;n++) ADC_low_pass[n] = 0;

  ADCSRA = (1<<ADPS1) | (1<<ADPS2) | (1<<ADPS0); // Prescale 128 .. @16Mhz ADC clock = 125kHz

  //ADCSRA = (1<<ADPS1) | (1<<ADPS2)  // Prescale 64 .. @16Mhz ADC clock = 250kHz
  ...
  DIDR2 |= 0xFF; 				/* disable ADC8-ADC15 digital in */
...
void A2D_poll_adc(void)
{
  ...
  if (bit_is_clear(ADCSRA, ADSC)){	// if clear, conversion is done.
	  ...
	  ADC_low_pass[chanNum] = ADC_low_pass[chanNum] - (ADC_low_pass[chanNum] >> LOW_PASS_BITS) + ADValue[chanNum];
...
{
	if (chanNum < 8)
		return (ADC_low_pass[chanNum] >> LOW_PASS_BITS);
	else
		return 0;
}
