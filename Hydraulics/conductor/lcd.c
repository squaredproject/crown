...
void lcd_init(void) { fprintf(LCDFILE, "\xfe\x53\x08\xfe\x46\xFE\x51"); }
...
void lcd_default(uint16_t knob[8], uint8_t mode) {
  ...
  lcd_line4();
  fprintf(LCDFILE, "AMP:%03u  %02u  PER:%03u", knob[0] >> 2, mode,
          (knob[4] >> 2) + 20);
}
...
void lcd_line1(void) { fprintf(LCDFILE, "\xfe\x46"); }

void lcd_line2(void) { fprintf(LCDFILE, "\xfe\x45\x40"); }

void lcd_line3(void) { fprintf(LCDFILE, "\xfe\x45\x14"); }

void lcd_line4(void) { fprintf(LCDFILE, "\xfe\x45\x54"); }
