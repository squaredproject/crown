...
void UART2_Init( unsigned int baud ) {
  //unsigned char x;
	  ...
	  UCSR2B = _BV(RXEN2)|_BV(TXEN2)|_BV(RXCIE2)|_BV(UDRIE2);  	// Enable rx and tx, and corresponding interrupts
	  UCSR2C = _BV(USBS2)|_BV(UCSZ20)|_BV(UCSZ21);				// Set frame format: 8data, 2stop bit
...
{
	return ( UART2_RxHead != UART2_RxTail ); /* Return 0 (FALSE) if the receive buffer is empty */
}
...
static int uart2_put_char(char c, FILE *stream)
{
	...
	tmphead = ( UART2_TxHead + 1 ) & UART2_TX_BUFFER_MASK; /* Wait for free space in buffer */
...
SIGNAL(USART2_UDRE_vect)
{
  ...
  if ( UART2_TxHead != UART2_TxTail ) {
    ...
    UCSR2B &= ~(1<<UDRIE2);   /* Disable UDRE interrupt or we'll re-trigger on exit */
  }
}
