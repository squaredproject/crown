...
Joint directions are denoted looking into the joint from the side of the
knuckleboard. Thus "LEFT" and "RIGHT."
...
Boards are connected so that a positive voltage comes from a pot control value
of > 63 while negative values are less than that.
...
thus active HIGH, but are mapped to a uint8 with positive logic such that if a
switch is closed the corresponding bit is set High. Bits are mapped, starting
from the LSB: LEFT CENTER RIGHT 0 0 0 0 0 and stored in the motor control struct
as "switches" variable.

There are three joints numbered 1, 2, and 3 from the bottom up. Many variables
for each joint are in a struct

LEDs
There are three status LEDs on the board, and two 'output' LEDs. The status LEDs
are Red, Green, and Yellow, and are used to indicate standard operation
(blinking green), error condition (red) plus additional status LEDs depending on
the specific error condition, or yellow (using potentiometer as input).

The two output LEDs are used to indicate a value between 0 and 127 (typically,
the signal that we're sending to one of the hydraulic valves.) The two output
LEDs are arranged in parallel but with opposite polarity, so that only one is
lit at a time. For values between 0-63, the left (green) LED is lit, and for
values over 63, the right (red) LED is lit. The intensity of the LED indicates
the degree of deviation from the 0 point (63).

Joint Enabling
Each driver board controls three hydraulic valves which are attached to their
respective joints. Output to the valves can be enabled (or disabled) by putting
a jumper to ground on pins A8-A10.

Code
The state of the system is largely contained in a structure called the Joint
Control Block (jcb) There is one jcb for each joint. It contains information on
the valve configuration and state (PI values, target values, current values);
joint max, min, and center values; homing and on/off state as well as other
values. Min and max values are *relative* to the center value.
...
// PORTK:  PK0-PK3 enable jumpers (adc12-adc15)     -Analog inputs for dial
// controls PORTL: (digital 48, 49) PLO=ICP4, PL1=ICP5 -- input compare for pos
// sense
...
uint8_t get_sine(uint16_t omega, double amp) {
  ...
  return (uint8_t)63 +
         lrint(63 * amp * ((((double)(sinval)) - ftablemax) / ftablemax));
}
...
void clear_status_LEDs(void) { PORTE &= ~(RED_LED | GRN_LED | YEL_LED); }
...
int main(void) {
  ...
  /* Note that this needs to go below the delay - there appears to be a race
     condition on read that affects some (but not all) boards */
  addr = (~PINB & 0x70) >> 4;
    ...
    if (KHZ_Flag) {
      ...
      if (tenhz_count++ > 980) { /* main loop at ~ten hz */
        ...
        if (ESTOP_PORT & _BV(ESTOP_PIN)) {
          ...
          if (omega0 >= TABLENGTH)
            omega0 -= TABLENGTH;
          target = get_sine(omega0, (double)A2D_read_channel(1) / 1024.0) - 63;
          ...
          for (i = 0; i < 3; i++) { /* for each joint, get inputs, set output */
            if ((enable & _BV(i)) &&
                jcb[i]->sw_enabled) { /* joint enabled from switches. XXX - also
                                         want a joint enabled via software. jcb
                                         enabled? */
              // Sanity test that the encoders are giving us good data
              if (Test_encoder_limits(counts[i])) { /* encoders in range ? */
                char buf[128];
                sprintf(buf, "\r\nEncoder limit error on joint %d, value %d", i,
                        counts[i]);
                putstr(buf);
                pause_error(ENCODER_ERROR);
              }
              // Another sanity test of encoder data. 'OF' is overflow - ie, we
              // haven't seen the expected edge on the PWM signal. Typically
              // this error will happen if the encoder is not physically
              // connected.
                ...
                uint8_t bLeftLimit =
                    ((softLimits & LEFT_SW) || (jcb[i]->switches & LEFT_SW));
                uint8_t bRightLimit =
                    ((softLimits & RIGHT_SW) || (jcb[i]->switches & RIGHT_SW));
...
void Dump_Status(void) {
  ...
  putstr(buf);
  sprintf(buf, "\r\n  Use Pot for Target : %s",
          (~ESTOP_PORT & _BV(6)) ? "TRUE" : "FALSE");
  putstr(buf);
  sprintf(buf, "\r\n  Estop Triggered    : %s",
          ((ESTOP_PORT & _BV(ESTOP_PIN)) ? "TRUE" : "FALSE"));
  putstr(buf);
  sprintf(buf, "\r\n  Error              : %s",
  ...
  for (int i = 0; i < 3; i++) {
    sprintf(buf, "\r\n  Joint %d Homed   : %s", i,
            (jcb[i]->homed ? "TRUE" : "FALSE"));
    putstr(buf);
  }
}

void setRunning(uint8_t bRunning) { isRunning = (bRunning == TRUE); }
...
static void sendJointLimits(uint8_t jointId) {
  CAN_SendJointLimits(jointId + 1, jcb[jointId]->minpos, jcb[jointId]->maxpos,
                      jcb[jointId]->center);
}
...
static void sendJointStatus(uint8_t jointId) {
  uint8_t enable = ~PINK;
  CAN_SendJointStatus(jointId + 1,
                      counts[jcb[jointId]->id] -
                          jcb[jointId]->center /*jcb[jointId]->currentPos*/,
                      jcb[jointId]->targetPos, jcb[jointId]->drive,
                      jcb[jointId]->switches, jcb[jointId]->homed,
                      (enable & _BV(jointId)) ? 1 : 0,
                      jcb[jointId]->sw_enabled);
}

static void sendExtendedStatus(void) {
  CAN_SendExtendedStatus(addr, 0, jcb[0]->homespeed, jcb[1]->homespeed,
                         jcb[2]->homespeed); // XXX no global debug level!
}

static void sendPIDValues(void) {
  CAN_SendPIDValues(jcb[0]->Kp, jcb[1]->Kp, jcb[2]->Kp, jcb[0]->Ki, jcb[1]->Ki,
                    jcb[2]->Ki);
}

static void sendIntegrators(void) {
  CAN_SendIntegrators(jcb[0]->integrator, jcb[1]->integrator,
                      jcb[2]->integrator);
}
