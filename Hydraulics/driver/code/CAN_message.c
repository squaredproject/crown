...
- ids of the devices. (Doesn't much matter. Just make them different. Make master have highest priority on the bus
...
general status              3   0   r|h  e   sw  je1 je2 je3  // bitfields r <<0, h<<1 . h is three bits. sw is two bits per joint sw0 << 0, sw1 << 2, sw2 << 4
valves                      4   0   v1   v1  v2  v2  v3  v3
joint status                5   j   p1   p1  v1  v1  e|h sw
homing status               6   j   p1   p1  sw  tar
...
unsigned long CAN_TX_MAP[] = {TOWER_1_TX_ID, TOWER_2_TX_ID, TOWER_3_TX_ID, TOWER_4_TX_ID};
...
int CAN_HomeJoint(uint8_t tower, uint8_t jointId) {
    ...
    cmd[CAN_TOWER_JOINT_BYTE] = ((tower & CAN_TOWER_MASK) << CAN_TOWER_SHIFT) | ((jointId & CAN_JOINT_MASK) << CAN_JOINT_SHIFT);
...
int CAN_SetRunState(uint8_t tower, int bRunning) {
    uint8_t cmd[8];
    uint8_t cmdLen = 2;
    cmd[CAN_CMD_BYTE]         = bRunning ? CAN_CMD_START_RUNNING : CAN_CMD_STOP_RUNNING;
...
int CAN_RequestJointStatus(uint8_t tower, uint8_t jointId) {
    ...
    cmd[CAN_TOWER_JOINT_BYTE] = ((tower & CAN_TOWER_MASK) << CAN_TOWER_SHIFT) | ((jointId & CAN_JOINT_MASK) << CAN_JOINT_MASK);
...
/*
int CAN_SetJointLimits(uint8_t tower, uint8_t jointId, int16_t minVal, int16_t maxVal, int16_t centerVal) {
    uint8_t cmd[8];
    uint8_t cmdLen = 8;
...
int CAN_SetHomeSpeed(uint8_t tower, uint8_t jointId, uint8_t speed) {
    ...
    cmd[CAN_TOWER_JOINT_BYTE] = ((tower & CAN_TOWER_MASK) << CAN_TOWER_SHIFT) | ((jointId & CAN_JOINT_MASK) << CAN_JOINT_SHIFT);
    cmd[CAN_DATA+0]       = speed > 63 ? 63 : speed;
...
int CAN_Neuter(uint8_t tower, uint8_t joint){
    ...
    cmd[CAN_TOWER_JOINT_BYTE] = ((tower & CAN_TOWER_MASK) << CAN_TOWER_SHIFT) | ((joint & CAN_JOINT_MASK) << CAN_JOINT_SHIFT);
    
    // push into CAN
    return (mcp_can_send(g_txId, 1, cmdLen, cmd));
}

int CAN_SetDriveLimits(uint8_t tower, uint8_t joint, int16_t minDrive, int16_t maxDrive) {
    uint8_t cmd[8];
    uint8_t cmdLen = 6;
    cmd[CAN_CMD_BYTE]         = CAN_CMD_SET_DRIVE_LIMITS;
    cmd[CAN_TOWER_JOINT_BYTE] = ((tower & CAN_TOWER_MASK) << CAN_TOWER_SHIFT) | ((joint & CAN_JOINT_MASK) << CAN_JOINT_SHIFT);
...
int CAN_SetPValue(uint8_t tower, uint8_t joint, int16_t pVal) {
    ...
    cmd[CAN_TOWER_JOINT_BYTE] = ((tower & CAN_TOWER_MASK) << CAN_TOWER_SHIFT) | ((joint & CAN_JOINT_MASK) << CAN_JOINT_SHIFT);
    cmd[CAN_DATA+0]       = (uint8_t)((pVal & 0xFF00) >> 8);
    cmd[CAN_DATA+1]       = (uint8_t)(pVal & 0x00FF);
...
int CAN_SetIValue(uint8_t tower, uint8_t joint, int16_t iVal) {
    ...
    cmd[CAN_TOWER_JOINT_BYTE] = ((tower & CAN_TOWER_MASK) << CAN_TOWER_SHIFT) | ((joint & CAN_JOINT_MASK) << CAN_JOINT_SHIFT);
    cmd[CAN_DATA+0]       = (uint8_t)((iVal & 0xFF00) >> 8);
    cmd[CAN_DATA+1]       = (uint8_t)(iVal & 0x00FF);
...
int CAN_SendHomingStatus(uint8_t jointId, int16_t pos, uint8_t sw, uint8_t target, uint16_t numStalls) {
    ...
    cmd[CAN_TOWER_JOINT_BYTE] = ((jointId & CAN_JOINT_MASK) << CAN_JOINT_SHIFT); 
    cmd[CAN_DATA+0]       = (uint8_t)((pos & 0xFF00) >> 8); // ?? network byte order? htons
    cmd[CAN_DATA+1]       = (uint8_t)(pos & 0x00FF);
    cmd[CAN_DATA+2]       = sw;
...
/*
/Users/carolyn/Library/Arduino15/packages/arduino/tools/avrdude/6.3.0-arduino6/bin/avrdude -C/Users/carolyn/Library/Arduino15/packages/arduino/tools/avrdude/6.3.0-arduino6/etc/avrdude.conf -v -patmega2560 -cwiring -P/dev/cu.usbmodem145141 -b115200 -D -Uflash:w:/var/folders/27/_ndl9qsj43l74vmkjbl8_ynr0000gn/T/arduino_build_228116/maquette.ino.hex:i 
REST API;

// Limits
GET /crown/limits      // get current limits for all towers  serial request:  <T1L>, <T2L>, <T3L> <T4L> serial response {limits : {towerid: right: left: center}}
PUT /crown/limits      // set limits for towers
GET /crown/savedLimits // get saved limits
PUT /crown/savedLimits // save new set of limits

#######

GET /crown/sculpture/(parameters, error state, homing state?) <T1S> <T2S> <T3S> <T4S>
GET /crown/sculpture/towers/[id] (returns position, limits, debug level, pid, error, run stat]
PUT /crown/sculpture/towers/[id]/[limits|debug|error|pid]  <T[id]S>
POST /crown/sculpture/towers/[id]/joint/[id]/home # XXX status???
PUT  /crown/sculpture/towers/[id]/joint/[id]/position

GET /crown/parameters/[pid|limits]

// send homing state on the bus... which joint we're homing, switches, targets, and values
