...
void UART1_Init( unsigned int baud ) { 
	  ...
	  UCSR1B = _BV(RXEN1)|_BV(TXEN1)|_BV(RXCIE1)|_BV(UDRIE1);  	// Enable rx and tx, and corresponding interrupts
	  UCSR1C = _BV(USBS1)|_BV(UCSZ10)|_BV(UCSZ11);				// Set frame format: 8data, 2stop bit
...
SIGNAL(USART1_UDRE_vect)
{
  ...
  if ( UART1_TxHead != UART1_TxTail ) {
    ...
    UCSR1B &= ~(1<<UDRIE1);   /* Disable UDRE interrupt or we'll re-trigger on exit */
  }
}
