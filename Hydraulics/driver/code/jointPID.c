...
static void finishHomingSetup(joint_control_block *j, uint16_t minpos_raw,
                              uint16_t maxpos_raw, uint16_t centerPos);
...
void Home_Joint(joint_control_block *j) {
  ...
  if (j->switches &
      LEFT_SW) { /* If we're already at the left limit, move right*/
    ...
    limit_mask = RIGHT_SW; /* ignore (pressed) left sw */
             ...
             RIGHT_SW) { /* If we're already at the right limit, move left */
    ...
    putstr("\r\nNo SW: default right");
    limit_mask =
        LEFT_SW | RIGHT_SW; /* change direction on hitting any limit sw */
  }

  /* look for home... We want to hit both of the limit switches and *then* the
     home switch... Note that we're also doing a little sanity checking - if we
     can't find home after a few back and forths, or the sculpture isn't
     responding to our commands, break out of the loop and set an error */
  int numLimits = 0;
  int numStalls = 0;
  int lastLimit = 0;
  uint16_t lastPos = counts[j->id];
  while ((limit_mask || !(j->switches & HOME_SW)) &&
         (numLimits <= MAX_LIMITS_IN_HOME) &&
         (numStalls <= MAX_STALLS_IN_HOME)) {
    ...
    if (j->switches /*& limit_mask*/) {
      ...
      if (j->switches & LEFT_SW && (lastLimit != LEFT_SW)) {
        ...
        limit_mask &=
            ~LEFT_SW; /* we've found the left switch - remove from objective */
        numLimits++;
        lastLimit = LEFT_SW;
        puthex(limit_mask);
        ...
        limit_mask &= ~RIGHT_SW; /* we've found the right switch - remove from
                                    objective */
        numLimits++;
        lastLimit = RIGHT_SW;
        puthex(limit_mask);
    ...
    if (1) {
      ...
      CAN_SendHomingStatus(j->id + 1, counts[j->id], j->switches, limit_mask,
                           numStalls);
    }
    ...
    if (diff >= STALL_MARGIN) {
      ...
      if ((speed > HOME_SPEED) &&
          (limit_mask &
           RIGHT_SW)) { // going right, and looking for right switch
        speed = HOME_SPEED - j->homespeed;
        limit_mask &= ~RIGHT_SW;
        maxpos_raw = counts[j->id] - 100;
        numStalls = 0;
        lastLimit = RIGHT_SW;
      } else if ((speed < HOME_SPEED) &&
                 (limit_mask &
                  LEFT_SW)) { // going left, looking for left switch
        speed = HOME_SPEED + j->homespeed;
        limit_mask &= ~LEFT_SW;
  ...
  if (numStalls > MAX_STALLS_IN_HOME) {
    ...
    /* went back and forth a few times without triggering the center switch.
     * Provisional pass */
    putstr("\r\n HOME failed. Center limit does not trigger. Will set to "
           "median of min/max");
    CAN_SendHomingResult(j->id + 1, 1, HOMING_RESULT_CENTER_NOT_FOUND,
                         limit_mask);
...
void Joint_Init_JCB(joint_control_block *j, uint8_t id) {
  ...
  // XXX - NB - the following max and min positions are derived from manually
  // homing the particular test rig that we have in the shop. In the final
  // version, these values should be found via the standard homing process
...
uint8_t Joint_Servo_Task(joint_control_block *j) {
  ...
  // Sanity check. If we're at or beyond the specified limits of motion, do
...
void Neuter_Joint(joint_control_block *j) {
  j->targetPos =
      counts[counts[j->id]] - j->center; // set target to current position
  j->drive = HOME_SPEED;
  j->integrator = 0;
}
